...
export default function Film() {
  ...
  const { data } = tmdb.movie.Details(Number(id));
  const { data: images } = tmdb.movie.Images(Number(id), { 
      ...
      <main className="main-container section-spacing pt-30 relative">
        ...
        <div className="w-fit min-h-120 flex flex-col gap-4 relative z-3">
          ...
          {!logo && poster && 
            ...
            />
            <span className="font-semibold text-xl mb-3">{data.title}</span>
            </>
          }
          <div className="max-w-screen flex flex-wrap gap-2">
            {data.genres.map((genre, index) => (
              <Genre key={'genre-id-' + genre + index} name={genre.name} link={'/filmy/zanr/' + genre.id} />
            ))}
          </div>
        ...
        <List header="Podobné filmy" type='movie' fetchFunction={(params) => tmdb.get("/movie/"+data.id+"/similar", params)} />
      </main>
    </ div>
  )
}