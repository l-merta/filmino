...
import { CollectionDetails, MediaDetails, Params } from "@/types/tmdbApi";

interface ListProps {
  header: string;
  icon?: React.ReactNode;
  type: "movie" | "tv";
  fetchFunction: (params?: Params) => Promise<CollectionDetails>;
  cardCount?: number;
}
  ...
  const fetchData = async (page: number = 1, increaseCardCount: boolean = true) => {
    try {
      ...
      if (newTotalCardCount > mediaItems.length) {
        ...
        setTotalPages(Math.floor(result.parts.length / 20) || 0);
        if (page === 1) {
          setMediaItems(result.parts || []);
        } else {
          setMediaItems(prev => [...prev, ...(result.parts || [])]);
        }
      }
    } catch (error) {
